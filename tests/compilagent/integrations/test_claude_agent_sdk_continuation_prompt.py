"""Unit tests for the Claude-Agent-SDK harness's continuation prompt builder."""

from __future__ import annotations

from compilagent.integrations.claude_agent_sdk.prompts import (
...
def test_continuation_uses_markdown_resume_header():
    text = continuation_user_prompt(_snap())
...
def test_continuation_skips_reflection_step_when_done():
    text = continuation_user_prompt(
        _snap(tools_called=frozenset({"synthesize_findings", "compare_runs"}))
    )
    assert "to reflect" not in text.lower() or "have learned so far" not in text


def test_continuation_lists_progress_state():
    text = continuation_user_prompt(
        _snap(
            successful_count=3,
            failed_attempts=2,
